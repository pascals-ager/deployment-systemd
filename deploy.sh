...
SERVER_IP="${SERVER_IP:-192.168.2.99}"
SSH_USER="${SSH_USER:-$(whoami)}"
KEY_USER="${KEY_USER:-$(whoami)}"
DOCKER_VERSION="${DOCKER_VERSION:-17.03.0}"

DOCKER_PULL_IMAGES=("postgres:9.4")
COPY_UNIT_FILES=("iptables-restore" "swap" "postgres" "python-flask-nginx" "nginx")
SSL_CERT_BASE_NAME="python-flask-nginx-example"
...
STAGING SERVER (DIRECT VIRTUAL MACHINE) DIRECTIONS:
  1. Configure a static IP address directly on the VM
     ...
     [change the last line to look like this, remember to set the correct
      gateway for your router's IP address if it's not 192.168.2.1]
...
function git_init () {
  ...
  scp "git/post-receive/python-flask-nginx" "${SSH_USER}@${SERVER_IP}:/tmp/python-flask-nginx"
  scp "git/post-receive/nginx" "${SSH_USER}@${SERVER_IP}:/tmp/nginx"
  ssh -t "${SSH_USER}@${SERVER_IP}" bash -c "'
sudo apt-get update && sudo apt-get install -y -q git
sudo rm -rf /var/git/python-flask-nginx.git /var/git/python-flask-nginx /var/git/nginx.git /var/git/nginx
sudo mkdir -p /var/git/python-flask-nginx.git /var/git/python-flask-nginx /var/git/nginx.git /var/git/nginx
sudo git --git-dir=/var/git/python-flask-nginx.git --bare init
sudo git --git-dir=/var/git/nginx.git --bare init

sudo mv /tmp/python-flask-nginx /var/git/python-flask-nginx.git/hooks/post-receive
sudo mv /tmp/nginx /var/git/nginx.git/hooks/post-receive
sudo chmod +x /var/git/python-flask-nginx.git/hooks/post-receive
sudo chmod +x /var/git/nginx.git/hooks/post-receive
sudo chown ${SSH_USER}:${SSH_USER} -R /var/git/python-flask-nginx.git /var/git/python-flask-nginx /var/git/nginx.git /var/git/nginx
  '"
  echo "done!"
}
...
sudo chmod +x /etc/systemd/system/${unit}.service
  '"
  done
  echo "done!"
}
...
function run_application () {
  ...
  ssh -t "${SSH_USER}@${SERVER_IP}" bash -c "'
sudo systemctl enable python-flask-nginx.service nginx.service
sudo systemctl start python-flask-nginx.service nginx.service
  '"
  echo "done!"
}
